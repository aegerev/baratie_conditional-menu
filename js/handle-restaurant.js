...
function handleGreetingAndDishRecommendation(currentHour) {
	...
	if(currentHour < 12) {
		...
		dishRecommendation = "Seafood Risotto";
	} else {
...
function handleDrinkRecommendation(greeting) {
	...
	switch(greeting) {
		case "Good Morning":
			...
			break;
		case "Good Afternoon":
			drinkRecommendation = "SmartWater";
			break;
		case "Good Evening":
			drinkRecommendation = "Ocean Spray Cranberry Juice Cocktail";
			break;
	}
...
function handleFridaySpecial(currentHour, currentDay) { 
    let specialDinnerCombo = "None";

	if(currentDay === 5 && currentHour >= 18){
        fridayBlock.style.display = "block";
        specialDinnerCombo = "Neptunian Penne Gorgonzola";
    } else {
...
function handleLoyaltyBonus(numVisitsThisMonth, currentHour) {
	...
	if(numVisitsThisMonth > 10) {
        if(currentHour < 18) {
            ...
            loyaltyBonus = "Complimentary Gramans (Grand Line Manju Buns)"
        }
    } else {
        loyaltyBonus = "Tony Tony Chopper Plush";
    }
...
function handleDateChange(currentHour, currentDay) {
	...
	dessertRecommendation = currentHour < 14 ? "Strawberry Muffin" : "Strawberry Cheesecake";
